import React, { useState } from 'react'
import Image from './Image'
import { FaRegHeart } from "react-icons/fa6";
import { FaRegEye } from "react-icons/fa6";
import { Link } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { Addtocart } from '../slices/CardSlice';
import { Wishlist, RemoveItem } from "../slices/WishSlice";


const Card = ({
  id,
  src1,
  Text,
  value1,
  value2,
  src2,
  num,
  className
}) => {

  const dispatch = useDispatch();
 

  const WishData = useSelector((state) => state.Wish.value);

const liked = WishData.some((item) => item.id === id);

  // Wishlist
const handleWish = () => {
  if (liked) {
    dispatch(RemoveItem(id));
  } else {
    dispatch(
      Wishlist({
        id,
        title: Text,
        image: src1,
        price: value1,
        price2: "$200",
      })
    );
  }
};

  // Add To Cart
  const handleAddtoCart = () => {
    dispatch(
      Addtocart({
        id,
        title: Text,
        price: value1,
        image: src1,
        Quantity: 1,
      })
    );
  };

  return (
    <div>
      <div className={`w-[300px] ${className}`}>

        <div className='w-full h-[280px] cursor-pointer bg-[#F5F5F5] rounded flex justify-center items-center flex-col relative overflow-hidden group'>

          <div>
            <Link to={`/ProductDetails/${id}`}>
              <Image src={src1} />
            </Link>
          </div>

          {/* Discount */}
          <div className='absolute top-3 left-3 bg-c1 px-3 py-1 rounded font-pop text-[12px] font-normal text-[#FAFAFA]'>
            -40%
          </div>

          {/* Wishlist */}
          <div
            onClick={handleWish}
            className={`w-[45px] h-[45px] rounded-full absolute right-3 top-[100%] flex justify-center items-center cursor-pointer group-hover:top-3 duration-400 ${
              liked ? "bg-c1 text-white" : "bg-white text-black"
            }`}
          >
            <FaRegHeart className='text-2xl' />
          </div>

          {/* Eye */}
          <div className='w-[45px] h-[45px] rounded-full bg-white absolute right-3 top-[100%] flex justify-center items-center cursor-pointer group-hover:top-18 duration-500'>
            <Link to={`/ProductDetails/${id}`}>
              <FaRegEye className='text-2xl' />
            </Link>
          </div>

          {/* Add To Cart */}
          <button
            onClick={handleAddtoCart}
            className='font-pop font-medium text-base text-white bg-black py-3 w-full absolute bottom-0 left-0 translate-y-full transition-all duration-400 group-hover:translate-y-0 rounded-b cursor-pointer'
          >
            Add To Cart
          </button>

        </div>

        <div className='w-full'>
          <Link to={`/ProductDetails/${id}`}>
            <h1 className='font-pop font-medium text-base text-black pt-[16px]'>
              {Text}
            </h1>
          </Link>

          <div className='py-[8px]'>
            <span className='text-c1 text-lg font-pop font-medium'>{value1}</span>
            <span className='text-black/50 text-lg font-pop font-medium ml-[12px]'>
              {value2}
            </span>
          </div>

          <div className='flex gap-x-[8px] items-center'>
            <Image src={src2} />
            <span className='text-sm text-black/50 font-pop font-semibold'>
              {num}
            </span>
          </div>

        </div>

      </div>
    </div>
  );
};

export default Card;